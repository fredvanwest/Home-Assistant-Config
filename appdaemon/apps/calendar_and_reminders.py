...
class calendar_and_reminders(hass.Hass):
    ...
    def initialize(self):
        # --- define variables ---
        # Note: Use local not duckdns!
        self.ha_url = "https://192.168.1.45:8123"
    ...
    def check_birthdays(self, kwargs):
        ...
        summaries = []
        dates_display = []
        _list = self.load_calendar("calendar.family_2",start_dt,end_dt)
        if _list == "error":
    ...
    def check_reminder(self, kwargs):
        ...
        start_dts = []
        end_dts = []
        _list = self.load_calendar("calendar.reminders",start_dt,end_dt)
        if _list == "error":
            ...
            for element in _list:
                ...
                if event_start_dt >= last_minute_dt and event_start_dt < end_check_interval_dt:
                    self.log("{} should I set as a reminder!".format(summary))
                    ...
                    self.log("{} probably did not start in this intervall".format(summary))
    ...
    def date_to_text(self, date_str):
        ...
        if days == 0:
            ...
            # Adjusted to day-of-week, mm/dd (n days)
            printtext = _date.strftime('{}').format(weekdays[_date.weekday()]) + ", " + _date.strftime('%m/%d ({} days)').format(days)
        return printtext
